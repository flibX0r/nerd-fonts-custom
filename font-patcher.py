...
class font_patcher:
    ...
    def patch(self):
        ...
        for patch in self.patch_set:
            if patch['Enabled']:
                ...
                if not SrcEnd:
                    SrcEnd = patch['SymEnd']
                self.copy_glyphs(SrcStart, SrcEnd, symfont, patch['SymStart'], patch['SymEnd'], patch['Exact'], patch['ScaleGlyph'], patch['Name'], patch['Attributes'], patch['GlyphPrefix'])
    ...
    def setup_arguments(self):
        ...
        parser.add_argument('-s', '--mono', '--use-single-width-glyphs', dest='single',           default=False, action='store_true', help='Whether to generate the glyphs as single-width not double-width (default is single-width)')
        ...
        sym_font_group.add_argument('--devicon',                                    dest='devicon',              default=True, action='store_false', help='Add Font Awesome Glyphs (https://fontawesome.com/)')
        sym_font_group.add_argument('--fontawesome',                                dest='fontawesome',          default=True, action='store_false', help='Add Devicons (https://github.com/devicons/devicon)')
        sym_font_group.add_argument('--fontlinux', '--fontlogos',                   dest='fontlinux',            default=True, action='store_false', help='Add Font Linux and other open source Glyphs (https://github.com/Lukas-W/font-logos)')
        sym_font_group.add_argument('--material', '--materialdesignicons', '--mdi', dest='material',             default=True, action='store_false', help='Add Material Design Icons (https://github.com/templarian/MaterialDesign)')
        
        self.args = parser.parse_args()

        # if you add a new font, set it to True here inside the if condition
        if self.args.complete:
            self.args.devicon = True
    ...
    def setup_font_names(self):
        verboseAdditionalFontNameSuffix = " " + projectNameAbbreviation
    ...
    def copy_glyphs(self, sourceFontStart, sourceFontEnd, symbolFont, symbolFontStart, symbolFontEnd, exactEncoding, scaleGlyph, setName, attributes, glyphPrefix=""):
        ...
        for index, sym_glyph in enumerate(symbolFont.selection.byGlyphs):
            ...
            self.sourceFont[currentSourceFontGlyph].glyphname = glyphPrefix + sym_glyph.glyphname
            scale_ratio_x = 1
            scale_ratio_y = 1
